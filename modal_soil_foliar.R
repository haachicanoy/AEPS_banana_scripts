...
write.csv(cosechasFinal, './DATOS_PROCESADOS/_cosecha/cosechas_suelo_foliar.csv', row.names=FALSE)
...
cosechasFinal <- read.csv('./DATOS_PROCESADOS/_cosecha/cosechas_suelo_foliarTest.csv')
# cosechasFinal <- read.csv('./DATOS_PROCESADOS/_cosecha/cosechas_suelo_foliarTest.csv')

cosechasFinal <- cosechasFinal[,c('Cluster',                                                     # Cosecha (CLUSTER PENDIENTE)
                                  'Arena_perc','Limo_perc','Arcilla_perc',                       # Suelo
                                  ...
                                  "Foliar_Perc_Sat.Ca","Foliar_Perc_Sat.Mg",                     # Foliar
                                  'Merma')]                                      # Cosecha (respuesta)

#'Area_finca',
# 'Longitud','Latitud',                                          # Cosecha
# 'Year_embolse','Semana_embolse',                               # Cosecha
# 'Year_cosecha','Mes_cosecha','Semana_cosecha',                 # Cosecha
# 'Embolsados_lote',                                             # Cosecha
# 'Recobro','Perc_corta','Perc_premio',                          # Cosecha (respuesta)
# 'Cajas_corta','Cajas_premio','Cajas_total',                    # Cosecha (respuesta)
# 'Racimos_cosechar_area','Peso_racimo',
...
# Load all functions
load('./RESULTADOS/Identificacion_factores_limitantes/_scripts/All-Functions-AEPS_BD.RData')

# Work Directory
dirFol  <- "./RESULTADOS/Identificacion_factores_limitantes/_results/_run5_merma_respond/"; setwd(dirFol)

# Database structure
cosechasFinal$splitVar <- 'All'

inputs  <- 1:38  # inputs columns
segme   <- 40    # split column
output  <- 39    # output column

dataSet   <- cosechasFinal
...
# Creating folders
createFolders(dirFol=getwd(), variety)

# Descriptive Analysis
descriptiveGraphics("All", dataSet, inputs=inputs, segme=segme, output=output, smooth=T, ylabel="Racimos cosechados/ha", smoothInd=NULL, ghrp="box")

# Datasets ProcesosF
dataSetProces(variety, dataSet, segme, corRed="caret")

# Multilayer perceptron
multilayerPerceptronFun(variety, dirLocation=paste0(getwd(),"/"), cualitInputs=FALSE, ylabs="Racimos cosechados/ha", pertuRelevance=TRUE, ncores=3) # All

# Random forest
# randomForestFun(variety, ncores=3) # All
randomForestFun("All", nb.it=30, ncores=3, saveWS=F)

# Conditional forest; especify if you have categorical variables
conditionalForestFun("All", nb.it=30, ncores=8, saveWS=F)
...
embolsados <- lapply(years, read_excel, path='//dapadfs/workspace_cluster_6/TRANSVERSAL_PROJECTS/MADR/COMPONENTE_2/ASBAMA/DATOS_BRUTOS/Info_Feliciana/Racimos_embolsados_2009-2015.xlsx')
for(i in 1:length(embolsados)){
  embolsados[[i]]$year <- paste(years[i])
}; rm(i)
embolsados <- Reduce(function(...) rbind(..., deparse.level=1), embolsados)
colnames(embolsados)[2:6] <- paste('L_', 1:5, sep='')
rownames(embolsados) <- 1:nrow(embolsados)
embolsados <- embolsados %>% gather(lote, racimos_embolsados, L_1:L_5)

write.csv(embolsados, '//dapadfs/workspace_cluster_6/TRANSVERSAL_PROJECTS/MADR/COMPONENTE_2/ASBAMA/DATOS_PROCESADOS/Feliciana_data_embolsados_fertilizacion.csv', row.names=F)

embolsados <- read.csv('//dapadfs/workspace_cluster_6/TRANSVERSAL_PROJECTS/MADR/COMPONENTE_2/ASBAMA/DATOS_PROCESADOS/Feliciana_data_embolsados_fertilizacion.csv')

# Fertilizantes

fertilizantes <- read_excel(path='//dapadfs/workspace_cluster_6/TRANSVERSAL_PROJECTS/MADR/COMPONENTE_2/ASBAMA/DATOS_BRUTOS/Info_Feliciana/Fertilizacion_2011-2015.xlsx', sheet=1)
fertilizantes <- fertilizantes[,c('Id_Lote','Año','Semana','Producto','Cantidad_kg')]
names(fertilizantes)[1:3] <- c('lote', 'year', 'Semana')
fertilizantes <- as.data.frame(fertilizantes)
fertilizantes$lote <- gsub(pattern='-', replacement='_', fertilizantes$lote)

# Para la tabla de embolse, 'Semana' corresponde a la semana de embolse
# Para la tabla de fertilizantes, 'Semana' corresponde a la semana de aplicación del fertilizante
# Cómo darse cuenta del momento del ciclo donde se encontraba (asumimos que se encontraba) el cultivo cuando se aplicó el fertilizante?

embolsados_fertilizantes <- merge(x=embolsados, y=fertilizantes, by=c('lote', 'year', 'Semana'), all.x=T)
embolsados_fertilizantes <- embolsados_fertilizantes[order(embolsados_fertilizantes$year, embolsados_fertilizantes$Semana),]
write.csv(embolsados_fertilizantes, '//dapadfs/workspace_cluster_6/TRANSVERSAL_PROJECTS/MADR/COMPONENTE_2/ASBAMA/DATOS_PROCESADOS/embolsados_fertilizantes_feliciana.csv', row.names=FALSE)


lm.present <- lm(racimos_embolsados~Cantidad_kg, data=embolsados_fertilizantes)
plot(x=embolsados_fertilizantes$Cantidad_kg, y=embolsados_fertilizantes$racimos_embolsados, pch=20, col=1)
abline(lm.present, col=1)
# points(x=dplyr::lag(embolsados_fertilizantes$Cantidad_kg, n=1), y=embolsados_fertilizantes$racimos_embolsados, pch=20, col=2)
# points(x=dplyr::lag(embolsados_fertilizantes$Cantidad_kg, n=2), y=embolsados_fertilizantes$racimos_embolsados, pch=20, col=3)
# points(x=dplyr::lag(embolsados_fertilizantes$Cantidad_kg, n=3), y=embolsados_fertilizantes$racimos_embolsados, pch=20, col=4)
# points(x=dplyr::lag(embolsados_fertilizantes$Cantidad_kg, n=4), y=embolsados_fertilizantes$racimos_embolsados, pch=20, col=5)
lm.lag5 <- lm(racimos_embolsados~dplyr::lag(Cantidad_kg, n=5), data=embolsados_fertilizantes)
points(x=dplyr::lag(embolsados_fertilizantes$Cantidad_kg, n=5), y=embolsados_fertilizantes$racimos_embolsados, pch=20, col=6)
abline(lm.lag5, col=6)
# points(x=dplyr::lag(embolsados_fertilizantes$Cantidad_kg, n=6), y=embolsados_fertilizantes$racimos_embolsados, pch=20, col=8)
# points(x=dplyr::lag(embolsados_fertilizantes$Cantidad_kg, n=7), y=embolsados_fertilizantes$racimos_embolsados, pch=20, col=9)
points(x=dplyr::lag(embolsados_fertilizantes$Cantidad_kg, n=10), y=embolsados_fertilizantes$racimos_embolsados, pch=20, col=2)
lm.lag10 <- lm(racimos_embolsados~dplyr::lag(Cantidad_kg, n=10), data=embolsados_fertilizantes)
abline(lm.lag10, col=2)

embolsados_fertilizantes$racimos_embolsados <- as.numeric(embolsados_fertilizantes$racimos_embolsados)

library(ggplot2)
plot <- ggplot(data=embolsados_fertilizantes, aes(x=lote, y=racimos_embolsados, fill=lote, colour=lote)) + geom_point()
plot

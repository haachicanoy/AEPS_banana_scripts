...
dataSet <- read.csv('./all_merma_clima_cycle.csv') # Change according database to analyse. It could be included climate, soils, foliar, etc. information
...
dataSet <- data.frame(dataSet[,1:(ncol(dataSet)-1)],
                      ...
                      Merma=dataSet[,ncol(dataSet)])
dataSet <- dataSet[dataSet$Merma>0,]
...
runID <- paste(wkDir, '/_runMerma_clima_cycle', sep='')

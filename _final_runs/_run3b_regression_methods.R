...
dataSet <- read.csv('./_cobana/cobana_fertilizaciones_clima_cycle.csv') # Change according database to analyse. It could be included climate, soils, foliar, etc. information
dataSet <- dataSet[,c(2:4, 9:ncol(dataSet), 5)]
...
inputs  <- 1:14  # inputs columns
segme   <- 15    # split column; In case of exists variety variable USE IT HERE
output  <- 16    # output column
